...
- Name: api/api-validation.spec.js >> M8 — API & Network Validation >> TC05 — PIM employee list API returns data @critical
- Location: tests/api/api-validation.spec.js:78:3
...
> 88  |     await page.goto(`${BASE_URL}/web/index.php/auth/login`);
  ...
  133 |   test('TC08 — Login page request contains required headers @sanity', async ({ request }) => {
  134 |     const response = await request.get(
  135 |       `${BASE_URL}/web/index.php/auth/login`,
  136 |       {
  137 |         headers: {
  138 |           'Accept': 'text/html',
  139 |           'User-Agent': 'Playwright-Enterprise-Test',
  140 |         },
  141 |       }
  142 |     );
  143 |     expect(response.status()).toBe(200);
  144 |     const contentType = response.headers()['content-type'];
  145 |     expect(contentType).toContain('text/html');
  146 |   });
  147 | 
  148 | });
```